"""
PD: Gemini Pro Image Generation Node (ComfyUI API Key)
Nano Banana Pro - 使用 ComfyUI API Key 调用 ComfyUI 代理 API
支持高分辨率图像生成（1K/2K/4K）
"""
...
def bytes_to_tensor(data):
    ...
    try:
        ...
        print(f"[GEMINI_PRO_APIKEY] Image decode error: {e}")
        return torch.zeros((1, 512, 512, 3))


class PDGeminiProImageGenAPIKey:
    """
    PD: Gemini Pro Image Generation (API Key)
    Nano Banana Pro - 使用 ComfyUI API Key 调用 ComfyUI 代理 API
    """
    ...
    def generate_image(self, api_key, prompt, model, aspect_ratio, resolution, response_modalities, seed, unique_id, images=None, files=None, system_prompt=""):
        """生成图片"""
        
        # 验证 API Key
        if not api_key or not api_key.strip():
            error_msg = "Error: ComfyUI API Key is required"
            print(f"[GEMINI_PRO_APIKEY] {error_msg}")
        ...
        if images is not None:
            ...
            for i in range(num_images):
                ...
                print(f"[GEMINI_PRO_APIKEY] Added reference image {i+1}/{num_images}")
            
            if images.shape[0] > 14:
                print(f"[GEMINI_PRO_APIKEY] Warning: Only first 14 images will be used (provided {images.shape[0]})")
        ...
        if files:
            for file_part in files:
                ...
                print(f"[GEMINI_PRO_APIKEY] Added file part")
        ...
        try:
            print(f"[GEMINI_PRO_APIKEY] Sending request to {model}")
            print(f"[GEMINI_PRO_APIKEY] Aspect Ratio: {aspect_ratio}, Resolution: {resolution}")
                ...
                for part in parts:
                    ...
                    if inline_data:
                        ...
                        if mime in ["image/png", "image/jpeg"] and data_b64:
                            ...
                            print(f"[GEMINI_PRO_APIKEY] Extracted image ({mime})")
            ...
            if not image_tensors:
                ...
                print(f"[GEMINI_PRO_APIKEY] {error_msg}")
                print(f"[GEMINI_PRO_APIKEY] Response: {str(result)[:500]}")
                return (torch.zeros((1, 512, 512, 3)), error_msg)
            ...
            print(f"[GEMINI_PRO_APIKEY] Success! Generated {len(image_tensors)} images in {duration:.2f}s")
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "PDGeminiProImageGenAPIKey": "PD: Gemini Pro Image (ComfyUI Key)"
}
